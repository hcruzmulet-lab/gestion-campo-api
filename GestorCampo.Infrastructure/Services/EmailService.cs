...
public class EmailService : IEmailService
{
    ...
    public Task SendEmailVerificationAsync(string toEmail, string name, string token, CancellationToken ct = default) =>
        SendAsync(toEmail, name, "Activa tu cuenta en GestorCampo",
            $"<p>Hola {name},</p><p>Activa tu cuenta haciendo clic en el siguiente enlace:</p><p><a href='{{APP_URL}}/verify-email?token={token}'>Activar cuenta</a></p><p>El enlace expira en 48 horas.</p>",
            ct);
    ...
    public Task SendPasswordResetAsync(string toEmail, string name, string token, CancellationToken ct = default) =>
        SendAsync(toEmail, name, "Recupera tu contraseña en GestorCampo",
            $"<p>Hola {name},</p><p>Recibimos una solicitud para restablecer tu contraseña:</p><p><a href='{{APP_URL}}/reset-password?token={token}'>Restablecer contraseña</a></p><p>El enlace expira en 24 horas. Si no solicitaste esto, ignora este mensaje.</p>",
            ct);
